...
resource "aws_autoscaling_group" "mygroup" {
  ...
  max_size                  = 3
  min_size                  = 2
  health_check_grace_period = 600 # Grace period is the time after which AWS EC2 instance comes into service before checking health.
  health_check_type         = "EC2" # The Autoscaling will happen based on health of AWS EC2 instance defined in AWS CLoudwatch Alarm 
